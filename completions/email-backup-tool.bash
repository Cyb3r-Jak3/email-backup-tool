...
__email-backup-tool_init_completion() {
  COMPREPLY=()
...
__email-backup-tool_build_completion_request() {
...
__email-backup-tool_find_description_for() {
  local candidate="$1"
  local i
...
__email-backup-tool_bash_autocomplete() {
  ...
  if [[ "${words[0]}" != "source" ]]; then
    ...
    __email-backup-tool_init_completion -n "=:" || return

    request_comp="$(__email-backup-tool_build_completion_request)"
    ...
    if [[ "${COMP_TYPE:-}" == "63" && ${#matches[@]} -gt 0 ]]; then
      ...
      for candidate in "${matches[@]}"; do
        local desc="$(__email-backup-tool_find_description_for "${candidate}")"
...
complete -o bashdefault -o default -F __email-backup-tool_bash_autocomplete email-backup-tool
